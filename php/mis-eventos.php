...
$guardarEvento = $pdo->prepare('INSERT INTO eventos (slug, titulo, categoria, ubicacion, imagen, fecha) VALUES (:slug, :titulo, :categoria, :ubicacion, :imagen, :fecha) ON DUPLICATE KEY UPDATE titulo = VALUES(titulo), categoria = VALUES(categoria), ubicacion = VALUES(ubicacion), imagen = VALUES(imagen), fecha = VALUES(fecha)');
foreach ($catalogo as [$slug, $titulo, $categoria, $ubicacion, $imagen, $fecha]) {
    $guardarEvento->execute(compact('slug', 'titulo', 'categoria', 'ubicacion', 'imagen', 'fecha'));
}
